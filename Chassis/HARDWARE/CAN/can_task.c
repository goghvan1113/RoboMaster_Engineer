...
/// @brief 从云台收到的底盘的速度三分量,Info[0~1]为工作状态信息,Info[2~3,4~5,6~7]分别为Vx、Vy、W速度三分量
uint8_t Info[8] = {0}; 
...
{
	CAN_Motor_Config( CHASSIS_MOTOR_1, CHASSIS_MOTOR_1_ID, CHASSIS_MOTOR_1_TYPE, CHASSIS_MOTOR_1_CH, CHASSIS_MOTOR_1_BIAS );  //1号GM3508电机
	CAN_Motor_Config( CHASSIS_MOTOR_2, CHASSIS_MOTOR_2_ID, CHASSIS_MOTOR_2_TYPE, CHASSIS_MOTOR_2_CH, CHASSIS_MOTOR_2_BIAS );  //2号GM3508电机
	CAN_Motor_Config( CHASSIS_MOTOR_3, CHASSIS_MOTOR_3_ID, CHASSIS_MOTOR_3_TYPE, CHASSIS_MOTOR_3_CH, CHASSIS_MOTOR_3_BIAS );  //3号GM3508电机
	CAN_Motor_Config( CHASSIS_MOTOR_4, CHASSIS_MOTOR_4_ID, CHASSIS_MOTOR_4_TYPE, CHASSIS_MOTOR_4_CH, CHASSIS_MOTOR_4_BIAS );  //4号GM3508电机
	CAN_Motor_Config( MINE_MOTOR_1, MINE_MOTOR_1_ID, MINE_MOTOR_1_TYPE, MINE_MOTOR_1_CH, MINE_MOTOR_1_BIAS );  //5号M2006电机
	CAN_Motor_Config( MINE_MOTOR_2, MINE_MOTOR_2_ID, MINE_MOTOR_2_TYPE, MINE_MOTOR_2_CH, MINE_MOTOR_2_BIAS );  //6号M2006电机
...
/**
 * @brief 将云台发送的数据,存储于全局变量之中
 * @param Stdid为数据发送方的识别符
 * @param Data[8]为数据发送方发送的数据
 */
void ReceiveGimbalData( uint32_t StdId, uint8_t Data[8] )
{  //云台发送的数据,Data[0~1]为工作状态信息,Data[2~3,4~5,6~7]分别为Vx、Vy、W速度三分量
	for( int i = 0; i <= 7; i++ )
	{
		Info[i] = Data[i];
	}
...
/**
 * @brief 获得云台发送的数据,并设置底盘的速度三分量
 * @param Data为存储云台数据的数组首地址
 */
void GetGimbalInfo( u8 *Data )
{
	//把云台通过CAN2传过来的信息存储在数组中
	Data[0] = Info[0];
	Data[1] = Info[1];
 ...
 * @brief 向云台发送信息
 * @param *msg为待发送信息的地址
 * @param len为待发送信息的长度,最大为8(第0位~第7位)
 * @return 电机序列号
 */
